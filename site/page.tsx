...
import "./unit.css";
import "./globals.css";
type View="home"|"units"|"recipes"|"reference";
...
${response.exit || "No response entered."}`;
 ...
 return <div className="shell">
  <header><button className="brand" onClick={()=>go("home")}><i>✦</i> GCSD Culinary Arts 1 & 2</button><nav>{(["home","units","recipes","reference"] as View[]).map(v=><button className={view===v?"active":""} onClick={()=>go(v)} key={v}>{v==="home"?"⌂ ":v==="units"?"▤ ":v==="recipes"?"♨ ":"☷ "}{v==="reference"?"Quick Reference":v[0].toUpperCase()+v.slice(1)}</button>)}</nav></header>
  <main>
